...
class Diffusion(nn.Module):
    ...
    def p_sample(self, x_t, t):  # p(x_(t-1) | x_t)
        ...
        x_t_minus_1 = unsqueeze_like(self.one_over_sqrt_alpha[t], predicted_noise) \
                      * (x_t - unsqueeze_like(self.beta_over_sqrt_root_one_minus_alpha_cumprod[t], predicted_noise) * predicted_noise) \
...
diffuser = Diffusion(denoiser=denoiser, num_timesteps=1000, img_size=IMG_SIZE, channels=3)

print(diffuser.get_simple_loss(torch.randn(8, 3, IMG_SIZE, IMG_SIZE)))
print(diffuser.sample())
